...
class TestProcessEventForOverlap(unittest.TestCase):
    ...
    def test_no_overlap_sequential_events(self):
        ...
        result_a = _process_event_for_overlap(event_a, state)
        ...
        result_b = _process_event_for_overlap(event_b, state)
    ...
    def test_partial_overlap_moves_to_overflow(self):
        ...
        result_b = _process_event_for_overlap(event_b, state)
        self.assertEqual(result_b["tid"], f"{_OVERFLOW_NAME_PREFIX}Stream 7")  # Should be moved
    ...
    def test_partial_overlap_small_overlap(self):
        ...
        result_b = _process_event_for_overlap(event_b, state)
        self.assertEqual(result_b["tid"], f"{_OVERFLOW_NAME_PREFIX}Stream 7")
    ...
    def test_non_x_phase_events_unchanged(self):
        ...
        event_a = {"name": "A", "ph": "X", "ts": 100.0, "dur": 100.0, "pid": "Device 0", "tid": "Stream 7", "cat": "kernel"}
        _process_event_for_overlap(event_a, state)
    ...
    def test_overflow_track_reused_after_gap(self):
        ...
        result_c = _process_event_for_overlap(event_c, state)
...
class TestWriteChromeTraceGzOverlapHandling(unittest.TestCase):
    ...
    def test_write_handles_partial_overlap(self):
        ...
        self.assertEqual(read_back["traceEvents"][1]["tid"], f"{_OVERFLOW_NAME_PREFIX}Stream 7")
...
class TestProcessChromeTraceFile(unittest.TestCase):
    """Test processing of existing Chrome trace files for overlap handling."""
    ...
    def test_process_trace_creates_backup_and_preserves_metadata(self):
        ...
        trace_data = {
            "displayTimeUnit": "ns",
            "traceEvents": [
                {"name": "A", "ph": "X", "ts": 100.0, "dur": 80.0, "pid": "Device 0", "tid": "Stream 7", "cat": "kernel"},
                {"name": "B", "ph": "X", "ts": 120.0, "dur": 80.0, "pid": "Device 0", "tid": "Stream 7", "cat": "kernel"},
            ],
        }
        ...
        tids = [e["tid"] for e in processed["traceEvents"] if e.get("ph") == "X"]
        self.assertEqual(tids[0], "Stream 7")
        self.assertEqual(tids[1], f"{_OVERFLOW_NAME_PREFIX}Stream 7")

    def test_process_trace_accepts_gzip_input_and_custom_output(self):
        """Processing should handle .json.gz input and honor custom output paths."""
        input_path = os.path.join(self.temp_dir, "input.json.gz")
        events = [
            {"name": "later", "ph": "X", "ts": 200.0, "dur": 30.0, "pid": "Device 0", "tid": "T0", "cat": "kernel"},
            {"name": "earlier", "ph": "X", "ts": 50.0, "dur": 10.0, "pid": "Device 0", "tid": "T0", "cat": "kernel"},
        ]
        with gzip.open(input_path, 'wt', encoding='utf-8') as f:
            json.dump({"traceEvents": events}, f)

        custom_output = os.path.join(self.temp_dir, "processed.json")
        result_path = process_chrome_trace_file(input_path, custom_output)

        self.assertEqual(result_path, custom_output)
        self.assertTrue(os.path.exists(custom_output))

        with open(custom_output, 'r', encoding='utf-8') as f:
            processed = json.load(f)

        # Events should be sorted by timestamp before processing
        names = [e["name"] for e in processed["traceEvents"]]
        self.assertEqual(names[0], "earlier")
        self.assertEqual(names[1], "later")
