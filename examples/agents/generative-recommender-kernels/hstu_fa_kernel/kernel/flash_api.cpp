...
at::Tensor cuda_hstu_mha(
    ...
    const std::optional<at::Tensor>& min_full_attn_seq_len_tensor =
        std::nullopt,
    int64_t num_groups = 1) {
  return hstu_ai_optimized::HSTUFlashAttentionFunctionGPU::apply(
      max_seq_len,
      alpha,
      q,
      k,
...
TORCH_LIBRARY_FRAGMENT(hstu_ai_optimized, m) {
  m.impl(
      "hstu_mha",
