"""
Build script for the reference HSTU attention kernel (hstu package).
Builds from generative-recommenders source tree without modifying it.
Uses corrected source file paths for the current branch.
Registers ops under torch.ops.hstu namespace.
"""
...
fa3_dir = os.path.dirname(this_dir)
repo_root = os.path.dirname(fa3_dir)
PACKAGE_NAME = "hstu"

# Reference source tree (absolute paths — we never modify this tree)
REF_CPP_DIR = os.path.join(
    repo_root, "generative-recommenders", "generative_recommenders", "ops", "cpp"
)
REF_ATTN_DIR = os.path.join(REF_CPP_DIR, "hstu_attention")
...
if not SKIP_CUDA_BUILD:
    ...
    cutlass_dir = Path(REF_CPP_DIR) / "cutlass"

    feature_args = (
        []
        + ["-DOSS_ENV"]  # Reference sources use #ifdef OSS_ENV guards
    ...
    # SM90 instantiations with softmax variants (absolute paths into reference tree)
    SOFTMAX = ["softmaxtrue", "softmaxfalse"]
    sources_fwd_sm90 = [
        os.path.join(
            REF_ATTN_DIR,
            f"instantiations/flash_fwd_hdim{hdim}_{dtype}_{softmax}_sm90.cu",
        )
    ...
    sources_bwd_sm90 = (
        [
            os.path.join(
                REF_ATTN_DIR,
                f"instantiations/flash_bwd_hdim{hdim}_{dtype}_{softmax}_sm90.cu",
            )
            for hdim, dtype, softmax in itertools.product(
                HEAD_DIMENSIONS, DTYPE, SOFTMAX
            )
        ]
    ...
    flash_api_cpu_src = os.path.join(REF_ATTN_DIR, "flash_api_cpu.cpp")

    sources = (
        [
            os.path.join(REF_ATTN_DIR, "flash_api.cpp"),
            os.path.join(REF_ATTN_DIR, "flash_common.cpp"),
            flash_api_cpu_src,
            os.path.join(REF_ATTN_DIR, "flash_common_cpu.cpp"),
        ]
        + sources_fwd_sm90
        + sources_bwd_sm90
    )
    ...
    include_dirs = [
        Path(REF_CPP_DIR),  # So #include "hstu_attention/..." works with -DOSS_ENV
        cutlass_dir / "include",
    ]
...
setup(
    ...
    description="FlashAttention HSTU (reference kernel)",
