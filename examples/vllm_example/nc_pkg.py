...
def install_vllm(compose_files: list[str], env: dict[str, str], name: str) -> None:
    """
    Install vllm in editable mode using the precompiled wheel.
    ...
    Args:
        ...
        name: Service name
    """
    # Find vllm wheel file using glob
    wheel_files = list(Path.cwd().glob("vllm*.whl"))
    
    if len(wheel_files) == 0:
        print("Error: No vllm*.whl file found in current directory", file=sys.stderr)
        sys.exit(1)
    elif len(wheel_files) > 1:
        print(f"Error: Multiple vllm wheel files found: {[f.name for f in wheel_files]}", file=sys.stderr)
...
def run_container(tag: str, name: str, auto_exec: bool = True):
    ...
    install_vllm(compose_files, env, name)
